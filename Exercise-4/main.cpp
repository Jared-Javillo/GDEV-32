...
struct Sphere : public SceneObject
{
    ...
    virtual float Intersect(const Ray& incomingRay,
                            ...
                            glm::vec3& outIntersectionNormal)
    {
        glm::vec3 oc = incomingRay.origin - center;
        //vd . vd
        float a = glm::dot(incomingRay.direction, incomingRay.direction);
        //2(vm . vd)
        float b = 2.0f * glm::dot(oc, incomingRay.direction);
        //vm.vm - r^2
        ...
        float t = std::min(t1, t2); // Take the smaller positive t
...
glm::vec3 RayTrace(const Ray& ray, const Scene& scene, glm::vec3& cameraPos, int depth)
{
    ...
    if (intersect.object != nullptr)
    {
        ...
        for ( auto& light : scene.lights)
        {
            ...
            {
                // point light
                lightDir = glm::normalize(intersect.intersectionPoint - lightPos);
                lightDistance = glm::distance(lightPos, intersect.intersectionPoint);
            }
